...
/// @return
/// 函数返回的是临时unique_ptr，因此是亡值，会调用移动构造函数（因此成立，unique_ptr没有拷贝构造）
...
WindowManager::WindowManager(Display* display)
    : display_(CHECK_NOTNULL(display)),
      ...
      WM_DELETE_WINDOW(
          XInternAtom(display_, "WM_DELETE_WINDOW",
                      false))  // 不存在就创建，并拿到服务器返回的原子标识
{}

WindowManager::~WindowManager() {
  XCloseDisplay(
      display_);  // WM并不决定X Client的声明周期，因此仅释放自己和自己的X连接
}
...
void WindowManager::Run() {
  ...
  //   a. 为根窗口指定一个错误处理回调，这样可以在多次启动WM时处理退出工作。
  {
    ...
    wm_detected_ = false;
    // 注册错误处理函数（此时在创建WM，是创建WM的错误处理函数）
    XSetErrorHandler(&WindowManager::OnWMDetected);
    // 在根窗口上注册子结构重定向，这个函数调用者是当前的WM，因此WM能监听根窗口及其直接子窗口的事件
    XSelectInput(
        display_, root_,
        SubstructureRedirectMask |
            SubstructureNotifyMask);  // NOTE -
                                      // 窗口管理器要能响应子结构重定向事件（手册上只有这两个与子结构重定向的事件掩码）
    XSync(
        display_,
        false);  // 刷新Xclient的输出缓冲区（发送request的）并等待Xserver处理完全部request，false表示不丢弃请求队列中的现有事件

    // 由于前面使用XSync刷新请求缓冲区，这样所有的事件都会被X
    // server处理。此时就能检查是否是多次启动WM了 Any errors generated must be
    // handled by the error handler. For each protocol error received by Xlib,
    // XSync() calls the client application's error handling routine. Any events
    // generated by the server are enqueued into the library's event queue.
    if (wm_detected_) {
  ...
  //   b. Set error
  //   handler.（此时已经完成了创建WM的过程，是WM创建后的错误处理函数）
  XSetErrorHandler(&WindowManager::OnXError);
  //   c. Grab X server to prevent windows from changing under us.
  // SECTION -
  // 使用了Grab操作来锁定Xserver，因为接下来要获取所有的窗口，此时不能让Xserver继续创建或销毁窗口
  ...
  CHECK(XQueryTree(display_, root_,
                   &returned_root,            // 根窗口ID
  ...
  //     因为后面不再需要这个数组了，XQueryTree()要求我们使用XFree()释放内存
  ...
  for (;;) {
    ...
    switch (e.type) {
      ...
      case ConfigureRequest:
        OnConfigureRequest(
            e.xconfigurerequest);  // 原模原样地为接受Xclient的调整窗口属性的请求，啥也没过滤
        break;
      case ButtonPress:
...
void WindowManager::Frame(Window w, bool was_created_before_window_manager) {
  ...
  // 我们应该只在它 可见(被Map了) 且 没有设置override_redirect
  // 的情况下才对它进行frame 前者加frame没有意义，后者不受WM管理
  ...
  // 3. Create frame.
  // 创建frame窗口同样也会触发CreateNotify事件，但同理，由于是Notify，其实WM没有什么要做的（即OnCreateNotify函数为空）
  ...
  // 使得w成为frame的子窗口
  XReparentWindow(display_, w, frame, 0,
                  0);  // Offset of client window within frame.
  // 7. Map frame.
  // 显示(Map)窗口的frame
  ...
  XGrabKey(display_, XKeysymToKeycode(display_, XK_F4), Mod1Mask, w, false,
           GrabModeAsync, GrabModeAsync);
  //   d. Switch windows with alt + tab.
  XGrabKey(display_, XKeysymToKeycode(display_, XK_Tab), Mod1Mask, w, false,
           GrabModeAsync, GrabModeAsync);
...
void WindowManager::Unframe(Window w) {
  ...
  XReparentWindow(display_, w, root_, 0,
                  0);  // Offset of client window within root.
...
void WindowManager::OnUnmapNotify(const XUnmapEvent& e) {
  ...
  // NOTE -
  // 忽略由WM启动之前map的窗口的reparent操作所触发的事件（这中情况发生在Run()，即WM刚刚创建时）
...
void WindowManager::OnConfigureRequest(const XConfigureRequestEvent& e) {
  XWindowChanges changes{.x = e.x,
                         ...
                         .stack_mode = e.detail};
  if (clients_.count(
          e.window)) {  // 如果该窗口拥有frame窗口，则frame窗口也需要改变config
...
void WindowManager::OnButtonPress(const XButtonEvent& e) {  // 鼠标按下事件
  ...
  CHECK(XGetGeometry(display_, frame, &returned_root, &x, &y, &width, &height,
                     &border_width, &depth));
...
void WindowManager::OnMotionNotify(const XMotionEvent& e) {
  ...
  LOG(WARNING) << "MotionNotify: " << clients_.size();
